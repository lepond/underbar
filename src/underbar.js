(function() {
  ...
  _.each = function(collection, iterator) {
    var key, i, length = collection.length;
    if (collection instanceof Array) {
      for (i = 0; i < length; i++) {iterator(collection[i], i, collection);}
    } else {
      for (key in collection) { iterator(collection[key], key, collection);}
    }
  };
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function (index){
      if (test(index)) {filtArray.push(index);
      }
    });
    return filtArray;
  };
  ...
  _.reject = function(collection, test) {
    return _.filter(collection, function (index) {return !test(index);});
  ...
  _.uniq = function(array) {
    var uniqArray = [];
    _.each(array, function(index) {if (_.indexOf(uniqArray, index) === -1) {uniqArray.push(index);
    }
  });
    return uniqArray;
  };
  ...
  _.map = function(collection, iterator) {
    var mappedArray = [];
    _.each(collection, function(index){ mappedArray.push(iterator(index)); });
    return mappedArray;
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var total;
    (accumulator === undefined) ? total = collection.shift() : total = accumulator;
     _.each( collection, function (index) { total = iterator(total, index); } );
     return total;
  };
  ...
  _.some = function(collection, iterator) {
    ...
    var iterator = iterator || _.identity;
    return _.reduce(collection, function (someTrue, index) {
      if (iterator(index) || someTrue) {
        return true;
      } else { return false;}
    }, false);
